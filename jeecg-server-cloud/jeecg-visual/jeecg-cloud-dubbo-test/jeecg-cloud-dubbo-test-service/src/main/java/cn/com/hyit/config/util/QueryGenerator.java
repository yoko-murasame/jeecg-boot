...
import cn.com.hyit.config.constant.SymbolConstant;
import cn.com.hyit.config.query.MatchTypeEnum;
import cn.com.hyit.config.query.QueryCondition;
import cn.com.hyit.config.query.QueryRuleEnum;
import cn.com.hyit.config.vo.SysPermissionDataRuleModel;
		...
		IntStream.range(0, columns.size()).forEach(i -> {
			...
			if (oConvertUtils.isNotEmpty(column) && oConvertUtils.isNotEmpty(order)) {
				...
				if (!allColumnExist(column, allFields)) {
					...
					log.warn("请注意，将要排序的列字段可能不存在：" + column);
				}
				...
				if (column.contains(",")) {
					...
					column = Optional.ofNullable(fieldColumnMap.get(column)).orElse(column);
				}
	...
	private static void addRuleToQueryWrapper(SysPermissionDataRuleModel dataRule, String name, Class propertyType, QueryWrapper<?> queryWrapper) {
		...
		if(rule.equals(QueryRuleEnum.IN) && ! propertyType.equals(String.class)) {
			...
			if (propertyType.equals(String.class)) {
				addEasyQuery(queryWrapper, name, rule, dataRule.getRuleValue());
			}else if (propertyType.equals(Date.class)) {
				String dateStr = dataRule.getRuleValue();
                int length = 10;
