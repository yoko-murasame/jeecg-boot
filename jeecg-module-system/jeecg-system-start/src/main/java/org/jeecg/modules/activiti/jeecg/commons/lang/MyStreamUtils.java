//
// Source code recreated from a .class file by IntelliJ IDEA
// (powered by FernFlower decompiler)
//

package org.jeecg.modules.activiti.jeecg.commons.lang;

import java.io.*;
import java.util.Arrays;
import java.util.List;

public class MyStreamUtils {
    static final int a = 4096;
    private static String[] b;

    public MyStreamUtils() {
    }

    public static String InputStreamTOString(InputStream in) {
        ByteArrayOutputStream var1 = new ByteArrayOutputStream();
        byte[] var2 = new byte[4096];
        String var3 = null;
        boolean var4 = false;

        int var9;
        try {
            while((var9 = in.read(var2, 0, 4096)) != -1) {
                var1.write(var2, 0, var9);
            }
        } catch (IOException var7) {
            var7.printStackTrace();
        }

        Object var8 = null;

        try {
            var3 = new String(var1.toByteArray(), "UTF-8");
        } catch (UnsupportedEncodingException var6) {
            var6.printStackTrace();
        }

        return var3;
    }

    public static String InputStreamTOString(InputStream in, String encoding) {
        String var2 = null;
        ByteArrayOutputStream var3 = new ByteArrayOutputStream();
        byte[] var4 = new byte[4096];
        boolean var5 = true;

        int var10;
        try {
            while((var10 = in.read(var4, 0, 4096)) != -1) {
                var3.write(var4, 0, var10);
            }
        } catch (IOException var8) {
            var8.printStackTrace();
        }

        Object var9 = null;

        try {
            var2 = new String(var3.toByteArray(), encoding);
        } catch (UnsupportedEncodingException var7) {
            var7.printStackTrace();
        }

        return var2;
    }

    public static InputStream a(String var0) throws Exception {
        ByteArrayInputStream var1 = new ByteArrayInputStream(var0.getBytes("UTF-8"));
        return var1;
    }

    public static byte[] StringTObyte(String in) {
        byte[] var1 = null;

        try {
            var1 = a(a(in));
        } catch (IOException var3) {
        } catch (Exception var4) {
            var4.printStackTrace();
        }

        return var1;
    }

    public static byte[] a(InputStream var0) throws IOException {
        ByteArrayOutputStream var1 = new ByteArrayOutputStream();
        byte[] var2 = new byte[4096];
        boolean var3 = true;

        int var5;
        while((var5 = var0.read(var2, 0, 4096)) != -1) {
            var1.write(var2, 0, var5);
        }

        Object var4 = null;
        return var1.toByteArray();
    }

    public static InputStream byteTOInputStream(byte[] in) throws Exception {
        ByteArrayInputStream var1 = new ByteArrayInputStream(in);
        return var1;
    }

    public static String a(byte[] var0) {
        InputStream var1 = null;

        try {
            var1 = byteTOInputStream(var0);
        } catch (Exception var3) {
            var3.printStackTrace();
        }

        return InputStreamTOString(var1, "UTF-8");
    }

    public static String b(String var0) {
        String var1 = null;

        try {
            var1 = a(StringTObyte(var0));
        } catch (Exception var3) {
            var3.printStackTrace();
        }

        return var1;
    }

    public byte[] b(InputStream var1) throws IOException {
        ByteArrayOutputStream var2 = new ByteArrayOutputStream();
        byte[] var3 = new byte[4096];
        boolean var4 = false;

        int var6;
        while((var6 = var1.read(var3, 0, 4096)) != -1) {
            var2.write(var3, 0, var6);
        }

        var2.flush();
        byte[] var5 = var2.toByteArray();
        return var5;
    }

    public static FileInputStream c(String var0) {
        FileInputStream var1 = null;

        try {
            var1 = new FileInputStream(var0);
        } catch (FileNotFoundException var3) {
            System.out.print("错误信息:文件不存在");
            var3.printStackTrace();
        }

        return var1;
    }

  /*  private static ResourceBundle c() {
        PropertyResourceBundle var0 = null;
        String var2 = System.getProperty("user.dir") + File.separator + "config" + File.separator + d.e();

        try {
            BufferedInputStream var1 = new BufferedInputStream(new FileInputStream(var2));
            var0 = new PropertyResourceBundle(var1);
            var1.close();
        } catch (FileNotFoundException var4) {
        } catch (IOException var5) {
        }

        return var0;
    }*/

    public static FileInputStream a(File var0) {
        FileInputStream var1 = null;

        try {
            var1 = new FileInputStream(var0);
        } catch (FileNotFoundException var3) {
            System.out.print("错误信息:文件不存在");
            var3.printStackTrace();
        }

        return var1;
    }

    public static FileOutputStream a(File var0, boolean var1) {
        FileOutputStream var2 = null;

        try {
            var2 = new FileOutputStream(var0, var1);
        } catch (FileNotFoundException var4) {
            System.out.print("错误信息:文件不存在");
            var4.printStackTrace();
        }

        return var2;
    }

    public static void a() {
        /*String var0 = "";

        try {
            if (b == null || b.length == 0) {
                ResourceBundle var1 = c();
                if (var1 == null) {
                    var1 = ResourceBundle.getBundle(d.d());
                }

                if (org.apache.commons.lang.b.b()) {
                    b = new String[]{e.a()};
                } else {
                    b = var1.getString(d.f()).split(",");
                }
            }

            if (!a(b, org.apache.commons.lang.b.d()) && !a(b, org.apache.commons.lang.b.a())) {
                System.out.println(d.h() + org.apache.commons.lang.b.c());
                String var6 = org.apache.commons.lang.a.c(var0, "123456");
                System.err.println(var6);
                System.exit(0);
            }
        } catch (Exception var5) {
            try {
                String var2 = org.apache.commons.lang.a.c(var0, "123456");
                System.err.println(var2);
                System.exit(0);
            } catch (Exception var4) {
            }
        }*/

    }

    public static FileOutputStream a(String var0, boolean var1) {
        FileOutputStream var2 = null;

        try {
            var2 = new FileOutputStream(var0, var1);
        } catch (FileNotFoundException var4) {
            System.out.print("错误信息:文件不存在");
            var4.printStackTrace();
        }

        return var2;
    }


    private static boolean a(String[] var0, String var1) {
        List var2 = Arrays.asList(var0);
        return var2.contains(var1);
    }

    public static File d(String var0) {
        return new File(var0);
    }

    public static ByteArrayOutputStream b() {
        return new ByteArrayOutputStream();
    }

    static {
        //a();
        b = null;
    }
}
