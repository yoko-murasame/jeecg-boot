...
public class DemoSQLConfig extends APIJSONSQLConfig {
	...
	static {
		DEFAULT_DATABASE = DATABASE_POSTGRESQL;  //TODO 默认数据库类型，改成你自己的。TiDB, MariaDB, OceanBase 这类兼容 MySQL 的可当做 MySQL 使用
		DEFAULT_SCHEMA = "public";  //TODO 默认数据库名/模式，改成你自己的，默认情况是 MySQL: sys, PostgreSQL: sys, SQL Server: dbo, Oracle:
		...
		tableColumnMap.put("POSTGRESQL-public-Privacy", Arrays.asList(StringUtil.split("id,certified,phone,balance,_password,_payPassword")));
		...
		tableKeyColumnMap.put("POSTGRESQL-public-Privacy", privacyKeyColumnMap);
	...
	public String getDBUri() {
		...
		if (isPostgreSQL()) {
			return "jdbc:postgresql://localhost:54321/apijson?stringtype=unspecified"; //TODO 改成你自己的
		}
		if (isSQLServer()) {
	...
	protected void onGetCrossJoinString(Join j) throws UnsupportedOperationException {
		// 开启 CROSS JOIN 笛卡尔积联表
		super.onGetCrossJoinString(j);
	}
	@Override
	protected void onJoinNotRelation(String sql, String quote, Join j, String jt, List<On> onList, On on) {
		// 开启 JOIN	ON t1.c1 != t2.c2 等不等式关联
		super.onJoinNotRelation(sql, quote, j, jt, onList, on);
	}
	@Override
	protected void onJoinComplextRelation(String sql, String quote, Join j, String jt, List<On> onList, On on) {
		// 开启 JOIN	ON t1.c1 LIKE concat('%', t2.c2, '%') 等复杂关联
		super.onJoinComplextRelation(sql, quote, j, jt, onList, on);
	}
